...
| Empty `rows` array but no error | No traffic in the last 7 days for those URLs, or regex matches nothing | Check GSC's "Performance" report filtered by URL. If traffic exists there but not in the last 7 days, the path is low-frequency — consider seeding the constants file manually. To extend the window, edit `getDateRange()` in your `scripts/run-gsc-prerender.mjs` (e.g. change `setDate(start.getDate() - 7)` to `- 14` for 2 weeks). If the path has no traffic at all, the regex or prefix is wrong. |
