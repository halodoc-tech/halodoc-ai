...
- They (or their platform team) need to schedule `npm run gsc-prerender` to run on a cadence (monthly is a good starting point) and open an MR/PR with the regenerated file. **This skill does not configure that scheduling.**
...
### 3.1 Locate the three files

Every change touches exactly these three:
...
The generated `gsc-prerender-params.constant.ts` is regenerated by the script — don't hand-edit it.
